...
unsigned long t0 = 0;
unsigned long t = 0;
unsigned long delaytime = 90000; //3 seconds
...
void loop()
{
  ...
  int temp_counter_stupid = 0;
  while(logdelay) {
    t=millis();
    delay(300);
    Serial.println("testing delay ");
    Serial.println(temp_counter_stupid);
    Serial.println("t = ");
    Serial.println(t);
    Serial.print("t0 = ");
    Serial.println(t0);
    ...
    temp_counter_stupid++;
    if (t-t0>=delaytime) {
      Serial.println("done with log delay");
      logdelay=false;
      logdata=true;
      EEPROMwriteEnablee();
      EEPROMchipErasee();
  ...
  if (logdata) {
    digitalWrite(led_red, LOW);
    Serial.println(" we have started logging data.");
